...
export type PromptInputRefineProps = PromptInputButtonProps & {
  onRefine?: (refinedPrompt: string) => void
}

export const PromptInputRefine = ({
  className,
  onRefine,
  ...props
}: PromptInputRefineProps) => {
  const controller = useOptionalPromptInputController()
  ...
  const handleRefine = async () => {
    // If already refining, cancel it
    if (isRefining && abortControllerRef.current) {
  ...
  return (
    <PromptInputButton
      ...
      disabled={!controller.textInput.value.trim() && !isRefining}
      onClick={handleRefine}
      title={isRefining ? "Cancel refinement" : "Refine Prompt"}
      {...props}
    >
      {isRefining ? (
