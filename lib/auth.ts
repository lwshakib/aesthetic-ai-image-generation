    ...
    sendResetPassword: async ({ user, url }) => {
      try {
        const { error } = await resend.emails.send({
          from: 'Aesthetic AI <noreply@lwshakib.site>',
          to: user.email,
          subject: 'Reset your password',
          react: AuthEmailTemplate({ type: 'forgot-password', url, userName: user.name }),
        });
    ...
    sendVerificationEmail: async ({ user, url }) => {
      try {
        await resend.emails.send({
          ...
          react: AuthEmailTemplate({ type: 'email-verification', url, userName: user.name }),
        });
      } catch (err) {
